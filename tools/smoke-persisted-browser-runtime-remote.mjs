...
const initialDelayMs = parsePositiveInteger(env.PERSISTED_BROWSER_RUNTIME_SMOKE_INITIAL_DELAY_MS, 30 * 1000);
const pollIntervalMs = parsePositiveInteger(env.PERSISTED_BROWSER_RUNTIME_SMOKE_POLL_INTERVAL_MS, 15 * 1000);
    ...
    JSON.stringify(
      {
        ...
        endpoint,
        target,
        message: "Set PROBE_API_KEY in the environment or .dev.vars before running remote persisted browser runtime smoke.",
      },
      null,
      2,
    ),
...
while (jobId && Date.now() - startedAt < timeoutMs) {
  ...
  const browserRuntimeJob = readProviderJob(latestPoll.body, "browser_runtime");
  if (isTerminalStatus(browserRuntimeJob?.status)) break;
...
function summarizeStatus(input) {
  ...
  const artifactOk = Boolean(
    input.artifact?.ok &&
      ...
      artifactHasRuntimeEvidence(input.artifact.body),
  );
  const browserRuntimeJob = readProviderJob(input.latestPoll?.body, "browser_runtime") ?? readProviderJob(input.read?.body, "browser_runtime");

  if (!createdOk || !readOk) return "failed";
  if (browserRuntimeJob?.status === "failed") return "failed";
  if (createdOk && readOk && artifactOk && browserRuntimeJob?.status === "completed") return "passed";
...
function summarizeArtifact(response) {
  ...
  return {
    ...
    runtime_record_count: records.filter(isRuntimeRecord).length,
    runtime_security_record_count: records.filter((record) => isRecord(record) && record.probe === "runtime_security_events_probe").length,
    runtime_api_record_count: records.filter((record) => isRecord(record) && record.probe === "runtime_api_requests_probe").length,
    runtime_bytes_record_count: records.filter((record) => isRecord(record) && record.probe === "runtime_resource_bytes_probe").length,
...
function artifactHasRuntimeEvidence(body) {
  const records = isRecord(body) && Array.isArray(body.records) ? body.records : [];
  return records.some((record) => isRecord(record) && record.probe === "runtime_security_events_probe");
}

function isRuntimeRecord(value) {
  return Boolean(isRecord(value) && typeof value.probe === "string" && value.probe.startsWith("runtime_"));
}
...
function writeSmokeResult(summary) {
  ...
  writeFileSync(resolve(dir, `persisted-browser-runtime-${safeTarget}-${stamp}.json`), `${JSON.stringify(summary, null, 2)}\n`);
}
