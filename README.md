...
- API base URL rendering verification: Worker version `4252efc4-4f90-4173-9cbe-60497e543d5e` passed the full selected AI report smoke against `https://poixe.com`: job `scan-poixe.com-d85c7e33-687e-47f6-b05c-99425f71d3ba`, 58 records, Layers 1-10, PageSpeed / Lighthouse / browser runtime / live TLS completed, 10 AI sections, exact JSON/direct Markdown match, no unknown refs, current public API base URL table rendered `https://api.poixe.com/`, `https://api-eu-central-1-dc8.poixe.com/`, and `https://api-eu-central-1-dc15.poixe.com/` without truncation, public API compatibility details still render `/v1/chat/completions`, `/v1/messages`, model naming, and provider routing, `Business model synthesis:` remains preserved, `model-load/provider routing` remains `/v1/models + provider-routing docs`, `/vendor/revenue` remains only a low-confidence derived alias, and unsupported `/vendor/log`, `/model?tab=model-load`, and old-reference-only `us-east-1.api.poixe.com` are not copied. Artifacts: `smoke-results/persisted-selected-full-ai-report-poixe.com-2026-05-25T05-56-30-759Z.json` and `.md`.
- Public API compatibility verification: Worker version `9e14702d-ef6c-425d-a12c-3b5e076b8fa6` passed the full selected AI report smoke against `https://poixe.com`: job `scan-poixe.com-b73f5031-a854-4603-85d9-eb1337d21800`, 58 records, Layers 1-10, PageSpeed / Lighthouse / browser runtime / live TLS completed, 10 AI sections, exact JSON/direct Markdown match, no unknown refs, `Business model synthesis:` preserved in both `Executive Summary` and `Organization and Operations Signals`, public API compatibility details rendered in API and Protocol Surface, OpenAI Chat Completions surfaced with `/v1/chat/completions`, Anthropic Messages surfaced with `/v1/messages`, model naming/provider routing docs selected, `model-load/provider routing` preserved as `/v1/models + provider-routing docs`, `/vendor/revenue` preserved only as a low-confidence derived alias, and unsupported `/vendor/log` plus `/model?tab=model-load` absent from the route table. Artifacts: `smoke-results/persisted-selected-full-ai-report-poixe.com-2026-05-25T05-32-09-879Z.json` and `.md`.
- Public business model synthesis verification: Worker version `a56bd157-e992-4c07-adf6-928708999f87` passed the full selected AI report smoke against `https://poixe.com`: job `scan-poixe.com-94a389a4-5db1-4966-a0f2-127706fa7866`, 57 records, Layers 1-10, PageSpeed / Lighthouse / browser runtime / live TLS completed, 10 AI sections, exact JSON/direct Markdown match, no unknown refs, `Business model synthesis:` present in both `Executive Summary` and `Organization and Operations Signals`, synthesis grounded in public content/detail/API/SPA-operation evidence, `model-load/provider routing` preserved as `/v1/models + provider-routing docs`, `/vendor/revenue` preserved only as a low-confidence derived alias, and unsupported `/vendor/log` plus `/model?tab=model-load` absent from the route table. Artifacts: `smoke-results/persisted-selected-full-ai-report-poixe.com-2026-05-25T04-48-53-751Z.json` and `.md`.
- SPA operation evidence synthesis verification: Worker version `d735004a-dac0-4a0a-b44f-0bac0dee1a7e` passed the full selected AI report smoke against `https://poixe.com`: job `scan-poixe.com-0bbfa4e8-72a6-4a12-a5cc-30bb8b994ac1`, 57 records, Layers 1-10, PageSpeed / Lighthouse / browser runtime / live TLS completed, 10 AI sections, exact JSON/direct Markdown match, no unknown refs, `SPA operation evidence table` present once, `model-load/provider routing` present once as `/v1/models + provider-routing docs`, `/vendor/revenue` present once as a low-confidence derived alias, and unsupported `/vendor/log` and `/model?tab=model-load` absent from the route table. Artifacts: `smoke-results/persisted-selected-full-ai-report-poixe.com-2026-05-25T03-38-54-634Z.json` and `.md`.
...
- Evidence prose compression verification: Worker version `c0226cfe-d72a-4050-8ca4-04388ccbd328` passed the full selected AI report smoke against `https://poixe.com`: job `scan-poixe.com-3acc2b45-7a8b-4608-8851-59e03155cc04`, 57 records, 10 AI sections, exact JSON/direct Markdown match, no unknown refs, raw inline `Evidence: key=value` prose reduced from 41 to 0 case-sensitive lines, concrete facts preserved, and duplicate performance/cookie/security/business-content paragraphs trimmed. Artifacts: `smoke-results/persisted-selected-full-ai-report-poixe.com-2026-05-24T14-39-29-328Z.json` and `.md`.
...
- Current gap ledger item: `G3 new_provider` / `G2 evidence_selection`.
- Current subtask: `G2.7 Remaining Target Gap Rebaseline`.
- Last completed subtask: `G2.6 Current Base URL / Regional Endpoint Evidence Rendering`.
- Next engineering focus: compare the latest generated poixe report after G2.6 against the target report, classify remaining gaps into `G1` through `G7`, and pick the next single actionable slice while preserving route strictness, business synthesis, operation evidence, brief retention, table ranking/filtering, boundary cleanup, and validated refs.
...
- 2026-05-25 poixe selected full-scan persisted AI report path is verified remotely for `G1.12 Public Business Model Synthesis`: Worker version `a56bd157-e992-4c07-adf6-928708999f87`, job `scan-poixe.com-94a389a4-5db1-4966-a0f2-127706fa7866`, 57 records, Layers 1-10, canonical 10-section AI Markdown, direct Markdown/JSON parity, `Business model synthesis:` present in both `Executive Summary` and `Organization and Operations Signals`, `model-load/provider routing` preserved as operation/capability evidence, `/vendor/revenue` preserved as a low-confidence derived alias, and unsupported exact aliases `/vendor/log` plus `/model?tab=model-load` still absent from the route table. Artifacts: `smoke-results/persisted-selected-full-ai-report-poixe.com-2026-05-25T04-48-53-751Z.json` and `smoke-results/persisted-selected-full-ai-report-poixe.com-2026-05-25T04-48-53-751Z.md`.
