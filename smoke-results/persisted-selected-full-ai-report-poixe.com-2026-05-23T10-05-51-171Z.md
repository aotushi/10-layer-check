...
The site's request and rendering chain involves a DNS lookup, followed by an HTTP request to the site's root URL. The browser then renders the site's HTML content, which includes a mix of static and dynamic resources.
...
The site has a subdomain attack surface, with 2 bounded HTTP(S) service fingerprint hints collected from 1 host. This may indicate exposed admin, staging, or tooling surfaces.
...
The site has a security posture that includes a frame embedding policy, but is missing security headers such as content-security-policy, strict-transport-security, and permissions-policy.
